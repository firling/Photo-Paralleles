...
const bookSchema = z.object({
  ...
  copiesTotal: z.number().int().min(0, "Le tirage doit être positif."),
  copiesRemaining: z.number().int().min(0, "Le stock doit être positif."),
  order: z.number().int(),
  published: z.boolean(),
...
function parseBookForm(
  ...
  const parsed = bookSchema.safeParse({
    ...
    copiesTotal: Number.parseInt(String(formData.get("copiesTotal") ?? "0"), 10) || 0,
    copiesRemaining:
      Number.parseInt(String(formData.get("copiesRemaining") ?? "0"), 10) || 0,
